...
contract('Game', accounts => {
  ...
  describe('initialisation', () => {
    it('should not be able to create a game with units greater than the field size', async () => {
      await Game.new('asdf', 2, 5, AUCTION_TIME, Team.red, { from: accounts[1] })
  ...
  describe.only('game without oracle', () => {
    ...
    beforeEach(async () => {
      instance = await Game.new('asdf', 2, 1, AUCTION_TIME, Team.red, { from: oracleAccount});
    });

    it('should not be able to make a move outside the field', async () => {
      await instance.placeBid(Team.red, [0, 2], { from: accounts[1], value: '1'})
        .catch((e: Error) => expect(e).not.to.be.null);

      await instance.placeBid(Team.red, [2, 0], { from: accounts[1], value: '1'})
        .catch((e: Error) => expect(e).not.to.be.null);
    });
    ...
    it('should be able to make a move', async () => {
      const tx = await instance.placeBid(Team.red, [0, 0], { from: accounts[1], value: '1'});
    ...
    it('should be able to make a move for the other team before the auction has started', async () => {
      ...
      await instance.placeBid(Team.blue, [0, 0], { from: accounts[2], value: '1'})
        .catch((e: Error) => expect(e).not.to.be.null);
    });

    it('should be able to outbid a move', async () => {
      const result1 = await instance.placeBid(Team.red, [0, 0], { from: accounts[1], value: '1'});
      const result2 = await instance.placeBid(Team.red, [0, 0], { from: accounts[2], value: '2'});
    });
    ...
    it('should be able to set the auction result and play again, once the other team has played', async () => {
      ...
      await instance.placeBid(Team.blue, [0, 1], { from: accounts[2], value: '1'});
      ...
      await instance.placeBid(Team.red, [0, 1], { from: accounts[1], value: '1'});
  ...
  describe('game with oracle', () => {
    ...
    it('should have the oracle call "startAuction"', async () => {
      ...
      await instance.placeBid(Team.blue, [0, 0], { from: accounts[1], value: '1'});
    });
  });
});